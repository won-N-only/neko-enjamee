...
const (
	videoSrc = "ximagesrc display-name=%s show-pointer=true use-damage=false ! video/x-raw,framerate=%d/1,colorimetry=sRGB ! videoconvert ! queue ! "
...
func NewVideoPipeline(rtpCodec codec.RTPCodec, display string, pipelineSrc string, fps int16, bitrate uint, hwenc config.HwEnc) (string, error) {
	...
	case codec.VP9().Name:
		...
		if hwenc == config.HwEncVAAPI {
			...
			pipelineStr = fmt.Sprintf(videoSrc+"video/x-raw,format=NV12 ! vaapivp9enc rate-control=cbr bitrate=%d keyframe-period=60 quality-level=5 ref-pic-mode=mode-0 ! video/x-vp9,profile=(string)0"+pipelineStr, display, fps, bitrate)
		} else {
