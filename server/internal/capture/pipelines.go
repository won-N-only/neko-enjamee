...
func NewVideoPipeline(rtpCodec codec.RTPCodec, display string, pipelineSrc string, fps int16, bitrate uint, hwenc config.HwEnc) (string, error) {
	...
	case codec.VP9().Name:
		...
		if hwenc == config.HwEncQSV {
			...
			pipelineStr = fmt.Sprintf(videoSrc+"video/x-raw,format=NV12 ! qsvvp9enc rate-control=vbr bitrate=%d target-usage=5 ! video/x-vp9,profile=(string)0"+pipelineStr, display, fps, bitrate)
		} else if hwenc == config.HwEncVAAPI {
			if err := gst.CheckPlugins([]string{"vaapi"}); err != nil {
				return "", err
			}

			pipelineStr = fmt.Sprintf(videoSrc+"video/x-raw,format=NV12 ! vaapivp9enc rate-control=vbr bitrate=%d keyframe-period=60 quality-level=5 ref-pic-mode=mode-1 ! video/x-vp9,profile=(string)0"+pipelineStr, display, fps, bitrate)
		} else {
	...
	case codec.H264().Name:
		...
		if hwenc == config.HwEncQSV {
			...
			pipelineStr = fmt.Sprintf(videoSrc+"video/x-raw,format=NV12 ! qsvh264enc rate-control=vbr bitrate=%d target-usage=5 ! video/x-h264,stream-format=byte-stream,profile=constrained-baseline"+pipelineStr, display, fps, bitrate)
		} else if hwenc == config.HwEncVAAPI {
			if err := gst.CheckPlugins([]string{"vaapi"}); err != nil {
				return "", err
			}

			pipelineStr = fmt.Sprintf(videoSrc+"video/x-raw,format=NV12 ! vah264enc rate-control=vbr bitrate=%d key-int-max=30 target-usage=4 ! video/x-h264,stream-format=byte-stream,profile=constrained-baseline"+pipelineStr, display, fps, bitrate)
