...
func NewVideoPipeline(rtpCodec codec.RTPCodec, display string, pipelineSrc string, fps int16, bitrate uint, hwenc config.HwEnc) (string, error) {
	...
	case codec.VP9().Name:
		...
		if hwenc == config.HwEncVAAPI {
			...
			pipelineStr = fmt.Sprintf(videoSrc+"video/x-raw,format=NV12 ! vaapivp9enc rate-control=cbr bitrate=%d keyframe-period=100 quality-level=5"+pipelineStr, display, fps, bitrate)
		} else {
